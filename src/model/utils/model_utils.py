...
def save_model(model: torch.nn.Module, file_name: str = 'fcn_resnet50_best_model', config: OmegaConf = None) -> None:
    '''
    summary:
    ...
    if is_best:
        ConfigManager.update_config(config=config, key='inference.checkpoint_path', value=save_path)
...
def load_model(model : torch.nn.Module, checkpoint_path : str, device : torch.device, library : str) -> torch.nn.Module:
    ...
    return :
        model : 로드된 모델
    '''
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=True) if library == 'torchvision' else smp.from_pretrained(checkpoint_path)
