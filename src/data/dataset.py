...
def check_image_label_pair(config) -> tuple[list[str], list[str]]:
    ...
    images = {
        osp.relpath(osp.join(root, fname), start = train_data_path)
    ...
    labels = {
        osp.relpath(osp.join(root, fname), start = train_label_path)
        for root, _, files in os.walk(train_label_path)
        for fname in files
        if osp.splitext(fname)[1].lower() == '.json'
    }

    jsons_fn_prefix = {osp.splitext(fname)[0] for fname in labels}
    pngs_fn_prefix = {osp.splitext(fname)[0] for fname in images}
...
def load_test_images(config):
    ...
    images = sorted([
        osp.relpath(osp.join(root, fname), start=test_data_path)
...
class XRayDataset(Dataset):
    ...
    def __init__ (self, mode='train', transforms=None, config=None):
        ...
        if mode in ['train', 'val']:
            ...
            left_right_group = [osp.dirname(fname) for fname in _imagenames]
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        if self.mode == 'test':
            image_path = osp.join(self.config.data.test_data_path, image_name)
            ...
            image_path = osp.join(self.config.data.train_data_path, image_name)
            image = cv2.imread(image_path)
            image = image / 255.0

            label_name = self.labelnames[idx]
            label_path = osp.join(self.config.data.train_label_path, label_name)
